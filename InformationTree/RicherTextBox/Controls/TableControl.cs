...
namespace RicherTextBox.Controls
{
    public partial class TableControl : UserControl
    {
        ...
        public TableControl(DataTable dataTableRef, RicherTextBox parent)
            : this()
        {
            this.parent = parent;
        ...
        private void TextBox_Resize(object sender, EventArgs e)
        {
            ...
            var calculatedHeight = parent.TextBox.Height - 25 - Location.Y;
            this.Height = this.Height > calculatedHeight ? this.Height : calculatedHeight;
        }

        public TableControl(string initializationData, RicherTextBox parent)
        ...
        public static DataTable GetDataTableFromString(string initializationData)
        {
            ...
            var lineZeroIsInfo = lines[0].StartsWith(TableSignature);

            string[] splitter;

            if (lineZeroIsInfo)
            {
                splitter = new string[1] { lines[0].Substring(TableSignature.Length) };
